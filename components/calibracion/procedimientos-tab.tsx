...
export function ProcedimientosTab({ procedimientosIniciales, canEdit }: Props) {
    ...
    try {
      if (editTarget) {
        ...
        if (pendingFile) {
          ...
          fd.append("folder", `procedimientos/${editTarget.id}`);
          const uploadRes = await fetch("/api/calibracion/upload", { method: "POST", body: fd });
          if (!uploadRes.ok) throw new Error("Error al subir archivo");
          const { url, nombre } = await uploadRes.json();
          updates.archivo_url = url;
          updates.archivo_nombre = nombre;
        }

        const res = await fetch(`/api/calibracion/procedimientos/${editTarget.id}`, {
          method: "PATCH",
        ...
        if (pendingFile) {
          const fd = new FormData();
          fd.append("file", pendingFile);
          fd.append("folder", `procedimientos/${created.id}`);
          const uploadRes = await fetch("/api/calibracion/upload", { method: "POST", body: fd });
          if (uploadRes.ok) {
            const { url, nombre } = await uploadRes.json();
            const patchRes = await fetch(`/api/calibracion/procedimientos/${created.id}`, {
              method: "PATCH",
              headers: { "Content-Type": "application/json" },
            ...
            if (patchRes.ok) {
              ...
              setProcedimientos((prev) => prev.map((p) => (p.id === patched.id ? patched : p)));
            }
          }
        }
      }
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/calibracion/procedimientos/${deleteTarget.id}`, {
        method: "DELETE",
      });
  ...
  async function handleFileUploadInline(proc: Procedimiento, file: File) {
    ...
    try {
      const fd = new FormData();
      fd.append("file", file);
      fd.append("folder", `procedimientos/${proc.id}`);
      const uploadRes = await fetch("/api/calibracion/upload", { method: "POST", body: fd });
      if (!uploadRes.ok) throw new Error("Error al subir archivo");
      const { url, nombre } = await uploadRes.json();
      const patchRes = await fetch(`/api/calibracion/procedimientos/${proc.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ archivo_url: url, archivo_nombre: nombre }),
      });
        ...
        <div className="flex flex-col items-center justify-center py-16 text-center rounded-lg border bg-white">
          ...
          {canEdit && (
            <p className="text-xs text-muted-foreground mt-1">
              Hacé clic en &ldquo;Nuevo procedimiento&rdquo; para agregar uno
            </p>
          )}
        </div>
      ) : (
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-muted/30">
                ...
                <TableHead className="w-[220px]">Archivo</TableHead>
                {canEdit && <TableHead className="w-[120px] text-right">Acciones</TableHead>}
              </TableRow>
            </TableHeader>
            <TableBody>
              {procedimientos.map((proc) => (
                <TableRow key={proc.id}>
                  ...
                  <TableCell className="text-sm text-muted-foreground">
                    {proc.descripcion || <span className="italic text-muted-foreground/60">Sin descripción</span>}
                  </TableCell>
                  <TableCell>
                    {proc.archivo_url ? (
                      <a
                        href={proc.archivo_url}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-1.5 text-xs text-primary hover:underline"
                      >
                        <ExternalLink className="h-3 w-3" />
                        {proc.archivo_nombre ?? "Ver archivo"}
                      </a>
                    ) : canEdit ? (
                      <div>
                        <input
                          type="file"
                          className="hidden"
                          id={`file-inline-${proc.id}`}
                          accept=".pdf,.doc,.docx,.xls,.xlsx"
                          onChange={(e) => {
                            const f = e.target.files?.[0];
                            if (f) handleFileUploadInline(proc, f);
                            e.target.value = "";
                          }}
                        />
                        <label htmlFor={`file-inline-${proc.id}`}>
                          {uploadingFor === proc.id ? (
                            <span className="inline-flex items-center gap-1 text-xs text-muted-foreground cursor-wait">
                              <Loader2 className="h-3 w-3 animate-spin" />
                              Subiendo...
                            </span>
                          ) : (
                            <span className="inline-flex items-center gap-1 text-xs text-muted-foreground cursor-pointer hover:text-primary">
        ...
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-1.5">
              <Label htmlFor="proc-titulo">Título *</Label>
              <Input
                id="proc-titulo"
                placeholder="Ej: Procedimiento de calibración de balanzas"
                value={titulo}
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex items-center gap-3">
                ...
                {!pendingFile && editTarget?.archivo_nombre && (
                  <span className="text-xs text-muted-foreground">
                    Actual: {editTarget.archivo_nombre}
                  </span>
                )}
              </div>
            </div>

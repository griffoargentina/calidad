...
interface Props { indicadores: Indicador[]; usuario: Usuario; }

const MESES_CORTOS = ["ENE","FEB","MAR","ABR","MAY","JUN","JUL","AGO","SEP","OCT","NOV","DIC"];

interface ModalState { open: boolean; indicadorId: string; indicadorNombre: string; metaCondicion: string | null; metaValor: string | null; anio: number; mes: number | null; frecuencia: string; }
...
function DataCell({ registro, isCurrentPeriod, canInput, onAdd }: { registro: Registro | undefined; isCurrentPeriod: boolean; canInput: boolean; onAdd: () => void; }) {
  if (registro) { const bgClass = registro.cumple === true ? "bg-green-50 text-green-800" : registro.cumple === false ? "bg-red-50 text-red-800" : "bg-slate-50 text-slate-600"; const displayVal = registro.valor.length > 8 ? registro.valor.slice(0, 8) + "…" : registro.valor; return <div className={cn("text-center text-xs font-medium px-1 py-1 rounded min-h-[28px] flex items-center justify-center", bgClass)} title={registro.valor + (registro.comentario ? " · " + registro.comentario : "")}>{displayVal}</div>; }
  if (isCurrentPeriod && canInput) return <button onClick={(e) => { e.stopPropagation(); onAdd(); }} className="w-full text-center text-xs text-primary hover:bg-primary/10 rounded py-1 min-h-[28px] flex items-center justify-center transition-colors" title="Cargar dato"><Plus className="h-3 w-3" /></button>;
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex flex-col gap-2 px-6 pt-4 pb-3 border-b">
        <div className="relative max-w-xs"><Search className="absolute left-2.5 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-slate-400" /><Input placeholder="Filtrar por título..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-8 h-8 text-xs" /></div>
      ...
      <Dialog open={modal.open} onOpenChange={(o) => setModal((m) => ({ ...m, open: o }))}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle className="text-base leading-snug">Cargar dato<span className="block text-xs font-normal text-slate-500 mt-0.5">{modal.indicadorNombre}</span>{modal.frecuencia === "mensual" && modal.mes && <span className="block text-xs font-normal text-slate-500">{MESES_CORTOS[modal.mes - 1]} {modal.anio}</span>}{modal.frecuencia === "anual" && <span className="block text-xs font-normal text-slate-500">Año {modal.anio}</span>}</DialogTitle></DialogHeader>
          ...
          <DialogFooter><Button variant="outline" size="sm" onClick={() => setModal((m) => ({ ...m, open: false }))}>Cancelar</Button><Button size="sm" onClick={handleSave} disabled={saving || !valorInput.trim()}>{saving ? "Guardando..." : "Guardar"}</Button></DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
  );

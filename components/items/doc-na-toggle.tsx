...
export function DocNaToggle({ itemId, value }: { itemId: string; value: boolean }) {
  ...
  async function toggle() {
    ...
    await fetch(`/api/items/${itemId}/quick-edit`, {
      ...
      body: JSON.stringify({ documento_na: !value }),
    });
    setLoading(false);
    router.refresh();
  }
  ...
  return (
    <Button size="sm" variant="outline" className="w-full text-xs" onClick={toggle} disabled={loading}>
      ...
      No corresponde documento para este item
    </Button>
  );
}
